...
{
    // Directory creation itself doesn't affect our file lists
    // FSMonitor will handle watching the new directory recursively
    // Any files inside will generate their own fileCreated events
    Q_UNUSED(path);
    Q_UNUSED(name);
}
...
{
    // Directory deletion doesn't need special handling
    // FSMonitor will generate fileDeleted events for files inside
    Q_UNUSED(path);
    Q_UNUSED(name);
}
...
void FSEventCollectorPrivate::handleDirectoryMoved(const QString &fromPath, const QString &fromName,
                                                   const QString &toPath, const QString &toName)
{
    // Directory moves don't need special handling
    // FSMonitor will generate appropriate file moved events for contents
    Q_UNUSED(fromPath);
    Q_UNUSED(fromName);
    Q_UNUSED(toPath);
    Q_UNUSED(toName);
